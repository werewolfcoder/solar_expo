...
//   const pointerGeometry = new THREE.SphereGeometry(0.5, 16, 16); // Adjust size here
...
// Function to reset the SolarScene to its initial state
function resetScene() {
  // Remove pointer if it exists
  if (pointer) {
      SolarScene.remove(pointer);
...
let orbitsVisible = true; // Track the visibility of orbits
...
document.getElementById('refreshBtn').addEventListener('click', resetScene);
...
//   const targetPosition = object.position.clone().add(new THREE.Vector3(0, 10, 30)); // Add an offset for better view

//   const startPosition = camera.position.clone();
//   const duration = 1.5; // Duration of the transition
//   let startTime = null;
...
//       } else {
//           controls.target.copy(object.position); // Lock onto the object (planet)
//           followingObject = object; // Begin following the selected object
//       }
//   }
...
// Add a variable to hold the highlight ring


// Function to create the highlight ring
